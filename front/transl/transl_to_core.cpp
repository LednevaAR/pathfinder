...
int main(int argc, char* argv[]) {
	...
	while (getline(file, st)) {
		...
		if (st.find("Node") != -1) {
			...
			if (mp.find(s1) == mp.end()) {	
				...
				} else if ((st.find("getelementptr") != -1) && ((st.find("i64 0, i64 0") != -1) || (st.find("i32 0, i32 0") != -1))) {//переписать
	...
	while (getline(file, st)) {
		if (st.find("->") != -1) {
			...
			if (marknode[s2] == "function") {
				s = "a";
			} else if ((marknode[s1] == "address") && (marknode[s2] == "mod_address")) {
				s = "d";
			...
			|| ((marknode[s1] == "address") && (marknode[s2] == "store"))) {
				s = "f";
			} else if (((marknode[s1] == "mod_address") && (marknode[s2] == "load"))
			|| ((marknode[s1] == "address") && (marknode[s2] == "load"))) {
				s = "n";
